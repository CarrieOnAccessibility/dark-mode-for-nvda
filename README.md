...
- Colour settings that preview live (the Settings dialog itself changes as you choose; Cancel puts it back): Background (dark grey, black, lighter grey, or a very dark red, orange, green, blue, purple, teal or pink, for dialogs, panels, lists, the Python console and the help pages; fields, buttons and menus unchanged), Accent (blue, red, orange, green, purple, teal, pink or yellow, for the focus rings, the menu outline, selected list rows, slider thumbs, and the check marks of check boxes and radio buttons; with Blue those two stay Windows' own), and "Bright contrast" (selected rows in the accent colour itself with black text). Any background goes with any accent.
