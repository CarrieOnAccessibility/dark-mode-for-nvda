...
- A solid focus ring on every control (check boxes, dropdowns, sliders and list rows included), an outline on the highlighted menu item, and a coloured selected row in lists; a "Focus outline thickness" slider makes the rings 1 to 4 pixels, drawn just outside the control with a small gap (like CSS outline-offset).
- Colour settings that preview live (the Settings dialog itself changes as you choose; Cancel puts it back): Background (dark grey, black, lighter grey, or a very dark red, orange, green, blue, purple, teal or pink, for dialogs, panels, lists, the Python console and the help pages; fields, buttons and menus unchanged), Accent (blue, red, orange, green, purple, teal, pink or yellow, for the focus rings, the menu outline, selected list rows, slider thumbs, and check boxes and radio buttons, which the add-on draws itself: white-bordered, with a bold tick), and "Bright contrast" (selected rows and check marks in the accent colour itself with black text, instead of white on the selected-row shade). A "Reset to defaults" button. Any background goes with any accent.
