...
- Colour settings that preview live (the Settings dialog itself changes as you choose; Cancel puts it back): Background (dark grey, black, lighter grey, or a very dark red, orange, green, teal, blue, purple or pink, for dialogs, panels, lists, the Python console and the help pages; fields, buttons and menus unchanged), Accent (blue, red, orange, green, purple, teal, pink or yellow, for the focus rings, the menu outline, selected list rows, slider thumbs, and check boxes and radio buttons, which the add-on draws itself with a light grey border and a bold tick; rings are the bright accent, selected rows a dark shade, boxes a middle one, slider thumbs between the two), and two "Bright contrast" check boxes (selected rows, and checkboxes and slider handles, in the bright accent with black text and marks). A "Reset to Dark Mode defaults" button. Any background goes with any accent.
